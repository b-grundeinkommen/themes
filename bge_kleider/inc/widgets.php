...
function BGE_kleider_widgets_init() {
    global $options;
   // Sidebar
    register_sidebar( array(
            'name' => __( 'Sitebar 1 (Upper)', 'BGE_kleider' ),
            'id' => 'sidebar-widget-area',
            'description' => __( 'This region is above of optional poster slider.', 'BGE_kleider' ),
   ...
   // Sidebar2
    register_sidebar( array(
            'name' => __( 'Sidebar 2 (Lower)', 'BGE_kleider' ),
            'id' => 'sidebar-widget-area-afterplakate',
            'description' => __( 'This region is below of optional poster slider.', 'BGE_kleider' ),
            'before_widget' => '<div class="widget">',
            'after_widget' => '</div>',
            'before_title' => '<h2>',
            'after_title' => '</h2>',
    ...
    register_sidebar( array(
            'name' => __( 'Start page: Intro', 'BGE_kleider' ),
            'id' => 'startpage-intro-area',
            'description' => __( 'Region below slider on start page.', 'BGE_kleider' ),
            'before_widget' => '<div class="widget">',
            'after_widget' => '</div>',
    ...
    register_sidebar( array(
            'name' => __( 'Start page: Left footer (content)', 'BGE_kleider' ),
            'id' => 'first-startpage-widget-area',
            'description' => __( 'On start page: Content footer, left side.', 'BGE_kleider' ),
            'before_widget' => '<div class="widget">',
            'after_widget' => '</div>',
            'before_title' => '<h3>',
            'after_title' => '</h3>',
    ...
    register_sidebar( array(
            'name' => __( 'Start page: Right footer (content)', 'BGE_kleider' ),
            'id' => 'second-startpage-widget-area',
            'description' => __( 'On start page: Content footer, right side.', 'BGE_kleider' ),
            'before_widget' => '<div class="widget">',
            'after_widget' => '</div>',
    ...
    register_sidebar( array(
            'name' => __( 'Page footer: Left', 'BGE_kleider' ),
            'id' => 'first-footer-widget-area',
            'description' => __( 'Region below main content, left site.', 'BGE_kleider' ),
            'before_widget' => '<div class="widget">',
            'after_widget' => '</div>',
            'before_title' => '<h2>',
            'after_title' => '</h2>',
    ...
    register_sidebar( array(
            'name' => __( 'Page footer: Right', 'BGE_kleider' ),
            'id' => 'second-footer-widget-area',
            'description' => __( 'Region below main content, right site.', 'BGE_kleider' ),
            'before_widget' => '<div class="widget">',
            'after_widget' => '</div>',
            'before_title' => '<h2>',
            'after_title' => '</h2>',
    ...
    // Widgets for indexpages (categories, tags, authorpage)
    register_sidebar( array(
            'name' => __( 'Index pages: Content footer', 'BGE_kleider' ),
            'id' => 'indexpages-widget-area',
            'description' => __( 'Content footer for index pages (e.g. categoryindex, archive, ...)', 'BGE_kleider' ),
            'before_widget' => '<div class="widget">',
            'after_widget' => '</div>',
            'before_title' => '<h3 class="widget-title">',
            'after_title' => '</h3>',
    ...
    if ($options['artikelstream-show-widget']==1) {
	 // Widgets for indexpages (categories, tags, authorpage)
    register_sidebar( array(
            'name' => __( 'Start page: Optional content', 'BGE_kleider' ),
            'id' => 'artikelstream-widget',
            'description' => __( 'Optional widget for content area; positioned after article stream and can be used to add external feeds in content area.', 'BGE_kleider' ),
...
add_action( 'widgets_init', 'BGE_kleider_widgets_init' );
...
class Newsletter_Widget extends WP_Widget {
	public function __construct() {
		parent::__construct(
			...
			__( 'Subscribe to newsletter', 'BGE_kleider' ),
			array( 'description' => __( 'Displays a form to subscribe to a mailing list.', 'BGE_kleider' ), ) // Args
		);
	}
		...
		echo $before_widget;				                    
                ...
                <form method="post" action="<?php echo $url; ?>">						
                    <label for="<?php echo $this->get_field_id( 'title' ); ?>">
                        <?php _e("Subscribe to newsletter", 'BGE_kleider'); ?>
                    </label>
                    <input type="text" id="<?php echo $this->get_field_id( 'title' ); ?>" 
                           name="email" 
                           value="<?php _e("Enter email address", 'BGE_kleider'); ?>" 
                           placeholder="<?php _e("Enter email address", 'BGE_kleider'); ?>"
                           onfocus="if(this.value=='<?php _e("Enter email address", 'BGE_kleider'); ?>')this.value='';" 
                           onblur="if(this.value=='')this.value='<?php _e("Enter email address", 'BGE_kleider'); ?>';">
                    <input type="submit" name="email-button" 
                           value="<?php _e("Subscribe", 'BGE_kleider'); ?>" id="newslettersubmit">
            <?php 	    
                $site_link = home_url();
                if ((isset($url))&& (strpos($url, $site_link) !== false)) {  
                    echo "<p>";
                    _e("Notice: You will leave this website for further steps.", 'BGE_kleider');
                    echo "</p>";
                } ?>		    
                </form>           
            </div>
	...
	public function form( $instance ) {
	    ...
	    ?> 
             <p>
                <label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Title:', 'BGE_kleider' ); ?>
                <input class="widefat" id="<?php echo $this->get_field_id( 'title' ); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" type="text" value="<?php echo esc_attr( $title ); ?>" />
                </label> 
            </p>

             <p>
                <label for="<?php echo $this->get_field_id( 'newsletter_url' ); ?>"><?php _e( 'URL for subscribing form (with email attribute)', 'BGE_kleider' ); ?>
	...
	public function __construct() {
		parent::__construct(
	 		'ParteiLinkliste_Widget', // Base ID
                        __( 'BGE Links', 'BGE_kleider' ),
			array( 'description' => __( 'List for several BGE party sections worldwide and in some countries', 'BGE_kleider' ), ) // Args
		);
	}
	...
	public function form( $instance ) {
		global $defaultoptions;
			...
			$bereich = $defaultoptions['default_footerlink_key'];
                ...
                global $default_footerlink_liste;
                echo "<label for=\"".$this->get_field_id( 'bereich' )."\">".__( 'Section:', 'BGE_kleider' )."</label>\n";
	...
	public function __construct() {
		parent::__construct(
	 		'Bannerlink_Widget', // Base ID
                        __( 'Image Link', 'BGE_kleider' ),
			array( 'description' => __( 'Sets an image link for media library', 'BGE_kleider' ), ) // Args
		);
	}
	public function form($instance) {
	    $defaults = array(
	    ...
	    ?>
            ...
            <p>
             ...
             <p>
                <label for="<?php echo $this->get_field_id('image_url'); ?>"><?php _e('Image:','BGE_kleider'); ?>
                       ...
                       type="text" value="<?php echo esc_attr($image_url); ?>" />
                        ...
                        <input class="button upload_image_button" name="upload_image_button" id="<?php echo $this->get_field_id('image_url'); ?>_button"  value="<?php _e('Upload', 'BGE_kleider'); ?>" />


                    <br /><?php _e('Select image from media library or enter an URL.', 'BGE_kleider'); ?>
