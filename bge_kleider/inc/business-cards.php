...
function BGE_kleider_person_post_type() {
	$labels = array(
		'name'                => _x( 'Business Card', 'Personal information', 'BGE_kleider' ),
		'singular_name'       => _x( 'Business Card', 'Personal information', 'BGE_kleider' ),
		'menu_name'           => __( 'Business Card', 'BGE_kleider' ),            
	);
	$args = array(
		'label'               => __( 'Business Card', 'BGE_kleider' ),
		'description'	      => __( 'Manage personal information', 'BGE_kleider' ),
		'labels'              => $labels,
...
add_action( 'init', 'BGE_kleider_person_post_type', 0 );

function BGE_kleider_taxonomies_person() {
	$labels = array();
	$args = array(
		'labels'	=> $labels,
		'hierarchical' => true,
...
add_action( 'init', 'BGE_kleider_taxonomies_person', 0 );
...
function BGE_kleider_person_metabox() {
    add_meta_box(
        'BGE_kleider_person_metabox',
        __( 'Personal information', 'BGE_kleider' ),
        'BGE_kleider_person_metabox_content',
        'person',
        'normal',
        'high'
    );
}
function BGE_kleider_person_metabox_content( $post ) {
	...
	?>
        ...
        <p>
		<label for="person_first_name"><?php _e( "Pre name", 'BGE_kleider' ); ?>:</label>
		<br />
	...
	<p>
		<label for="person_last_name"><?php _e( "Last name", 'BGE_kleider' ); ?>:</label>
		<br />
        ...
        <p>
		<label for="person_academic"><?php _e( "Academic title", 'BGE_kleider' ); ?>:</label>
		<br />
        ...
        <p>
		<label for="person_shortdesc"><?php _e( "Short description", 'BGE_kleider' ); ?>:</label>
		<br />
	...
	<p>
	    <label for="person_bild"><?php _e( "Personal image", 'BGE_kleider' ); ?>:</label>
	    <br />
		...
		?>
	     ...
	     <input type="button" id="person_bild-button" class="button" value="<?php _e( "Select or upload image", 'BGE_kleider' ); ?>" />
	    <small> <a href="#" class="custom_clear_image_button"><?php _e("Remove image",'BGE_kleider'); ?></a></small> 
	</p>
	
	<p>
		<label for="person_email"><?php _e( "Email", 'BGE_kleider' ); ?>:</label>
		<br />
		<input  type="text" name="person_email"
			id="person_email" value="<?php echo esc_attr( get_post_meta( $post->ID, 'person_email', true ) ); ?>" size="30" />
	</p>
        <p>
		<label for="person_pgp_fingerprint"><?php _e( "PGP Fingerprint", 'BGE_kleider' ); ?>:</label>
		<br />
	...
	<p>
		<label for="person_url"><?php _e( "Homepage (URL)", 'BGE_kleider' ); ?>:</label>
		<br />
	...
	<p>
		<label for="person_wiki"><?php _e( "Wiki Page (Username)", 'BGE_kleider' ); ?>:</label>
		<br />
	...
	<p>
		<label for="person_twitter"><?php _e( "Twitter (Accountname)", 'BGE_kleider' ); ?>:</label>
		<br />
	...
	<p>
		<label for="person_facebook"><?php _e( "Facebook (URL)", 'BGE_kleider' ); ?>:</label>
		<br />
	...
	<p>
		<label for="person_google"><?php _e( "Google+ (URL)", 'BGE_kleider' ); ?>:</label>
		<br />
	...
	<p>
		<label for="person_friendica"><?php _e( "Friendica (URL)", 'BGE_kleider' ); ?>:</label>
		<br />
	...
	<p>
		<label for="person_instagram"><?php _e( "Instagram (URL)", 'BGE_kleider' ); ?>:</label>
		<br />
	...
	<p>
		<label for="person_newsfeed"><?php _e( "Personal Newsfeed (URL)", 'BGE_kleider' ); ?>:</label>
		<br />
...
add_action( 'add_meta_boxes', 'BGE_kleider_person_metabox' );


function BGE_kleider_person_metabox_save( $post_id ) {
    global $options;
...
add_action( 'save_post', 'BGE_kleider_person_metabox_save' );


function BGE_kleider_person_metabox_updated_messages( $messages ) {
	global $post, $post_ID;
	$messages['person'] = array(
		0 => '',
		1 => __('Business card updated.', 'BGE_kleider'),
		2 => __('Business card updated.', 'BGE_kleider'),
		3 => __('Business card removed.', 'BGE_kleider'),
		6 => __('Business card publiced.', 'BGE_kleider'),
		7 => __('Business card saved.', 'BGE_kleider'),
			);
	return $messages;
}
add_filter( 'post_updated_messages', 'BGE_kleider_person_metabox_updated_messages' );





function BGE_kleider_display_person ($post_id = 0, $format = 'full', $profillink = 1) {
    global $options;
	...
	if (isset($person_url) || isset($person_email) 
	    ...
	    $kontaktdata .= '<h3 class="contact">'.__('Contact','BGE_kleider').'</h3>';
		...
		$kontaktdata .= "<li class=\"website\"><span>Web: </span><a class=\"extern\" itemprop=\"url\" href=\"".$person_url."\">".BGE_kleider_display_url($person_url)."</a></li>\n";
	    }
		...
		$kontaktdata .= "<li class=\"twitter\"><span>Twitter: </span><a href=\"".$url."\">".BGE_kleider_display_url($url)."</a></li>\n";
	    }
	     if (isset($person_facebook) && strlen($person_facebook)>1) {
		$kontaktdata .= "<li class=\"facebook\"><span>Facebook: </span><a href=\"".$person_facebook."\">".BGE_kleider_display_url($person_facebook)."</a></li>\n";
	    }
	    if (isset($person_google) && strlen($person_google)>1) {
		$kontaktdata .= "<li class=\"google\"><span>Google: </span><a href=\"".$person_google."\">".BGE_kleider_display_url($person_google)."</a></li>\n";
	    } 
             if (isset($person_friendica) && strlen($person_friendica)>1) {
		$kontaktdata .= "<li class=\"friendica\"><span>Friendica: </span><a href=\"".$person_friendica."\">".BGE_kleider_display_url($person_friendica)."</a></li>\n";
	    } 
             if (isset($person_instagram) && strlen($person_instagram)>1) {
		$kontaktdata .= "<li class=\"instagram\"><span>Instagram: </span><a href=\"".$person_instagram."\">".BGE_kleider_display_url($person_instagram)."</a></li>\n";
	    } 
		...
		$kontaktdata .= "<li class=\"".$wikiclass."\"><span>Wiki: </span><a href=\"".$url."\">".BGE_kleider_display_url($url)."</a></li>\n";
	    } 
             if (isset($person_newsfeed) && strlen($person_newsfeed)>1) {
		$kontaktdata .= "<li class=\"feed\"><span>Feed: </span><a href=\"".$person_newsfeed."\">".BGE_kleider_display_url($person_newsfeed)."</a></li>\n";
	    } 
	    $kontaktdata .= "</ul>\n";
	...
	if ($format== 'full') {
	    ...
	    $out .= '<h3 class="about">'.__('About','BGE_kleider').' ';
	    ...
	    $out .= "</div>\n";
           ...
           if ($options['vcard-showfeed']==1 && isset($person_newsfeed) && strlen($person_newsfeed)>1) {
                ...
                if ($limit == 0) {
                    $out .= '<div class="personfeed skip">'.__("The feed is either empty or unavailable.",'BGE_kleider').'</div>';
                } else {
                     $out .= '<div class="personfeed">';
                     $out .= "<h3>".__('Last posts', 'BGE_kleider')."</h3>\n";
                     $out .= "<ul>";
            ...
            if ($options['vcard-showlocalentries']==1) {
                ...
                if ($personposts->have_posts() ) { 
                    ...
                    while ($personposts->have_posts()) : $personposts->the_post();
                        $ppout = BGE_kleider_post_teaser(1,1,1,200, 1,1,4);        
                         if (isset($ppout)) {
                            $ppoutput .= $ppout;
                         }
                     endwhile;
                ...
                if (isset($ppoutput)) {
                    ...
                    $out .=  "<h3>".__('Last entries from this site about', 'BGE_kleider')." $fullname</h3>\n";
                    $out .= $ppoutput;
                    $out .= "</div>\n"; 
                }
            }
...
function BGE_kleider_person_shortcode( $atts ) {
    global $options;
		...
		if( $person->have_posts() ) {
		    ...
		    while ($person->have_posts() ) {
			    ...
			    if (isset($id) && isset($format) &&($format=='short')) {
				    $out .= BGE_kleider_display_person($post_id, 'short');
				    ...
				    $out .= BGE_kleider_display_person($post_id, 'bigtable');
		            } else {
				     $out .= BGE_kleider_display_person($post_id, 'full');
			    }
			}
			if ((isset($format) && ($format=='table') && ($single==0)) 
			     || (isset($format) && ($format=='bigtable') && ($single==0))) {
			...
			$out .= __('No person found.', 'BGE_kleider');
			$out .= "</p></section>\n";
		}
	 wp_reset_query();
	return $out;
}
add_shortcode( 'person', 'BGE_kleider_person_shortcode' );


/* Adding Metabox for setting a link from posts to people */

/* Fire our meta box setup function on the post editor screen. */
add_action( 'load-post.php', 'BGE_kleider_post_metabox_person_setup' );
add_action( 'load-post-new.php', 'BGE_kleider_post_metabox_person_setup' );

/* Meta box setup function. */
function BGE_kleider_post_metabox_person_setup() {
	/* Add meta boxes on the 'add_meta_boxes' hook. */
	add_action( 'add_meta_boxes', 'BGE_kleider_add_post_metabox_person' );	
		/* Save post meta on the 'save_post' hook. */
	add_action( 'save_post', 'BGE_kleider_save_post_class_meta', 10, 2 );
}
/* Create one or more meta boxes to be displayed on the post editor screen. */
function BGE_kleider_add_post_metabox_person() {

	add_meta_box(
		'BGE_kleider-post-class-person',			// Unique ID
		esc_html__( 'Business card', 'BGE_kleider' ),		// Title
		'BGE_kleider_post_class_metabox_person',		// Callback function
...
function BGE_kleider_post_class_metabox_person( $object, $box ) { 
	global $defaultoptions;
	
	wp_nonce_field( basename( __FILE__ ), 'BGE_kleider_post_class_nonce' ); 
	?>
	<p>
		<label for="BGE_kleider-personalcard-id"><?php _e( "Select someone you want to show with a business card along the article.", 'BGE_kleider' ); ?></label>
		<br />
		<select name="BGE_kleider-personalcard-id" id="BGE_kleider-personalcard-id">
		    <option value=""><?php _e( "Do not display any business card", 'BGE_kleider' ); ?></option>
		    <?php
		    
			$notice = '';
			 $oldid = esc_attr( get_post_meta( $object->ID, 'BGE_kleider-personalcard-id', true ) );
		    	    $args = array(
					'post_type' => 'person',
					'order' => 'ASC',
					'meta_key' => 'person_last_name',
				...
				$notice = __('No business cards defined yet.', 'BGE_kleider');
			    }
			    wp_reset_query();
			    if (isset($out)) {
				echo $out;
	...
	<p>
	    <?php _e( "Alternativly define manually a short text and an image:", 'BGE_kleider' ); ?>
	</p>
	
	<p>
		<label for="BGE_kleider-sidebar-text"><?php _e( "Short text for sidebar", 'BGE_kleider' ); ?></label>
		<br />
		<?php 
		    // Downwards comtability:  Version 2 used "text" as meta key, Version 3 "BGE_kleider-sidebar-text"
		    $textfield = esc_attr( get_post_meta( $object->ID, 'BGE_kleider-sidebar-text', true ) );
		    if (empty($textfield)) {
			$textfield = esc_attr( get_post_meta( $object->ID, 'text', true ) );			
		    }
		  ?>
		
		<input class="widefat" type="text" name="BGE_kleider-sidebar-text" id="BGE_kleider-sidebar-text" value="<?php echo $textfield; ?>" size="30" />
	</p>
	
	<p>
	    <label for="person_bild"><?php _e( "Picture", 'BGE_kleider' ); ?>:</label>
	    <br />
	     	    
		<?php
		
		 $person_bildid = get_post_meta( $object->ID, 'BGE_kleider-sidebar-image_id', true );
		 $person_bild = get_post_meta( $object->ID, 'BGE_kleider-sidebar-image_url', true );
		...
		?>
	     ...
	     <input type="button" id="person_bild-button" class="button" value="<?php _e( "Select or upload image", 'BGE_kleider' ); ?>" />
	    <small> &nbsp;  <a href="#" class="custom_clear_image_button"><?php _e( "Remove picture", 'BGE_kleider' ); ?></a></small> 
	</p>
	</div>
...
function BGE_kleider_save_post_class_meta( $post_id, $post ) {

	/* Verify the nonce before proceeding. */
	if ( !isset( $_POST['BGE_kleider_post_class_nonce'] ) || !wp_verify_nonce( $_POST['BGE_kleider_post_class_nonce'], basename( __FILE__ ) ) )
		return $post_id;
	...
	$newid = ( isset( $_POST['BGE_kleider-personalcard-id'] ) ? sanitize_key( $_POST['BGE_kleider-personalcard-id'] ) : '' );
	$oldid = get_post_meta( $post_id, 'BGE_kleider-personalcard-id', true );

	if ( $newid && '' == $oldid )
		add_post_meta( $post_id, 'BGE_kleider-personalcard-id', $newid, true );
	elseif ( $newid && $newid != $oldid )
		update_post_meta( $post_id, 'BGE_kleider-personalcard-id', $newid );
	elseif ( '' == $newid && $oldid )
		delete_post_meta( $post_id, 'BGE_kleider-personalcard-id', $oldid );
	

	$newid = ( isset( $_POST['person_bildid'] ) ? sanitize_key( $_POST['person_bildid'] ) : '' );
	$oldid = get_post_meta( $post_id, 'BGE_kleider-sidebar-image_id', true );
	if (empty($oldid))  {
	    $oldid =get_post_meta( $post_id, 'image_url', true );
	}
	if ( $newid && '' == $oldid )
		add_post_meta( $post_id, 'BGE_kleider-sidebar-image_id', $newid, true );
	elseif ( $newid && $newid != $oldid )
		update_post_meta( $post_id, 'BGE_kleider-sidebar-image_id', $newid );
	elseif ( '' == $newid && $oldid )
		delete_post_meta( $post_id, 'BGE_kleider-sidebar-image_id', $oldid );	
	...
	$oldurl = get_post_meta( $post_id, 'BGE_kleider-sidebar-image_url', true );
	
	if ( $bildurl && '' == $oldurl )
		add_post_meta( $post_id, 'BGE_kleider-sidebar-image_url', $bildurl, true );
	elseif ( $bildurl && $bildurl != $oldurl )
		update_post_meta( $post_id, 'BGE_kleider-sidebar-image_url', $bildurl );
	elseif ( '' == $bildurl && $oldurl )
		delete_post_meta( $post_id, 'BGE_kleider-sidebar-image_url', $oldurl );	
	
	
	$new_text = ( isset( $_POST['BGE_kleider-sidebar-text'] ) ? sanitize_text_field( $_POST['BGE_kleider-sidebar-text'] ) : '' );
	$oldtext = get_post_meta( $post_id, 'BGE_kleider-sidebar-text', true );
	if (empty($oldtext)) {
	    $oldtext = get_post_meta( $post_id, 'text', true );
	}
	if ( $new_text && '' == $oldtext )
		add_post_meta( $post_id, 'BGE_kleider-sidebar-text', $new_text, true );
	elseif ( $new_text && $new_text != $oldtext )
		update_post_meta( $post_id, 'BGE_kleider-sidebar-text', $new_text );
	elseif ( '' == $new_text && $oldtext )
		delete_post_meta( $post_id, 'BGE_kleider-sidebar-text', $oldtext );	
...
if ( ! function_exists( 'get_BGE_kleider_steckbrief' ) ) :
    /*
     * Anzeige der Steckbrief-Info zu einem Post
     */
    
function get_BGE_kleider_steckbrief(){
  global $post;
  global $options;
  
  
  $personid = esc_attr( get_post_meta( $post->ID, 'BGE_kleider-personalcard-id', true ) );
  if ((isset($personid)) && ($personid>0)) {      
      return BGE_kleider_display_person($personid,'sitebar'); 
  }  
  
  $text = esc_attr( get_post_meta( $post->ID, 'BGE_kleider-sidebar-text', true ) );
   if (empty($text)) {
       /* Downwards compatibility */
	$text = esc_attr( get_post_meta( $post->ID, 'text', true ) );			
    }
   $image_url = get_post_meta( $post->ID, 'BGE_kleider-sidebar-image_url', true );
   $image_id = 0;
    if (empty($image_url))  {
	/* Downwards compatibility */
	$image_url = get_post_meta( $post->ID, 'image_url', true );
    } else {
	$image_id = get_post_meta( $post->ID, 'BGE_kleider-sidebar-image_id', true );
    }
...
function BGE_kleider_display_url($url = '') {
