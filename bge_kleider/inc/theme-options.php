...
function theme_options_init(){
	register_setting( 'BGE_kleider_options',
                'BGE_kleider_theme_options', 
...
function theme_options_add_page() {
	add_theme_page( __( 'Theme Options', 'BGE_kleider' ),
                        __( 'Theme Options', 'BGE_kleider' ), 
                       'edit_theme_options', 
                        'theme_options', 
...
function theme_options_do_page($tab = '') {
	...
	<div class="wrap">            
            <div class="BGE_kleider-optionen">  <!-- begin: .BGE_kleider-optionen -->    
            <?php echo "<h2>" . wp_get_theme().': ' . __( 'Change theme options', 'BGE_kleider' ) . "</h2>"; ?>

            <?php if ( false !== $_REQUEST['settings-updated'] ) : ?>
            <div class="updated fade"><p><strong><?php _e( 'Options saved.', 'BGE_kleider' ); ?></strong></p></div>
            <?php endif; 
        ...
        if (!isset($setoptions['BGE_kleider_theme_options'][$tab])) {
            echo "Invalid Tab-Option or undefined Option-Field $tab";            
        }        

        
        echo "<h3 class=\"nav-tab-wrapper\">\n";
        foreach($setoptions['BGE_kleider_theme_options'] as $i => $value) {                
        ...
        <form method="post" action="options.php">
            <?php settings_fields( 'BGE_kleider_options' ); ?>
            <input type="hidden" id="BGE_kleider_theme_options[tab]" name="BGE_kleider_theme_options[tab]"  value="<?php echo $tab; ?>">                       
          
          
        <div id="einstellungen">                                       
	<table>	
                <?php
                    if (isset($setoptions['BGE_kleider_theme_options'][$tab]['fields'])) {
                        foreach($setoptions['BGE_kleider_theme_options'][$tab]['fields'] as $i => $value) {   
                            $name = $i;
			    $mark_option =0;
				...
				echo "\t\t\t<table class=\"suboptions\">\n";      
			       ...
			       echo "\">\n\t\t<th scope=\"row\">";
                               ...
                               echo "</th>\n\t\t<td>";
                                ...
                                if ($type =='bool') {
                                    echo "\t\t\t";
                                    echo "<input id=\"$name\" name=\"BGE_kleider_theme_options[$name]\" 
				    ...
				    echo ">\n";
                                    ...
                                    echo "\t\t\t";
                                    echo "<input class=\"regular-text\" id=\"BGE_kleider_theme_options[$name]\" 
                                            type=\"text\" name=\"BGE_kleider_theme_options[$name]\" 
					    value=\"";
				    if (isset($options[$name])) echo esc_attr( $options[$name] );				
				    
				    echo "\"><br>\n";
                                    echo "\t\t\t";
                                    echo "<label for=\"BGE_kleider_theme_options[$name]\">$label</label>\n";
				} elseif (($type=='html') ||($type=='url')) {
                                    echo "\t\t\t";
                                    echo "<input class=\"large-text\" id=\"BGE_kleider_theme_options[$name]\" 
                                            type=\"text\" name=\"BGE_kleider_theme_options[$name]\" 
					    size=\"120\" value=\"";
				    if (isset($options[$name])) echo esc_attr( $options[$name] );				
				    ...
				    if (isset($options[$name])) {
					    echo "<img class=\"imgurl\" src=\"".esc_attr( $options[$name] )."\" alt=\"\">\n";
                                    ...
                                    echo "\t\t\t";                                                                                                            
                                    echo "<textarea class=\"large-text\" id=\"BGE_kleider_theme_options[$name]\" 
                                            cols=\"30\" rows=\"10\"  name=\"BGE_kleider_theme_options[$name]\">";
				    if (isset($options[$name])) echo esc_attr( $options[$name] );
				    echo "</textarea><br>\n";
                                    echo "\t\t\t";
                                    echo "<label for=\"BGE_kleider_theme_options[$name]\">$label</label>\n";    
				    ...
				    <input type="hidden" name="BGE_kleider_theme_options[<?php echo $name; ?>]" id="file_<?php echo $name; ?>" 
					     value="<?php if (isset($options[$name])) echo sanitize_key( $options[$name]); ?>" />
				    
				    <input type="text" name="BGE_kleider_theme_options[<?php echo $name; ?>_url]" id="file_<?php echo $name; ?>_url" 
					   value="<?php if (isset($options[$name])) echo wp_get_attachment_url( esc_attr( $options[$name]) ); ?>" />
				    <input class="button" name="file_button_<?php echo $name; ?>" id="file_button_<?php echo $name; ?>" value="<?php _e('Add file', 'BGE_kleider'); ?>" />
				    <small><a href="#" class="file_remove_<?php echo $name; ?>"><?php _e( "Remove file", 'BGE_kleider' );?></a></small>
				    <br><label for="BGE_kleider_theme_options[<?php echo $name; ?>]"><?php echo $label; ?></label>
				    </div><script>
					...
					$addstyle .= 'max-height: '.$value['maxheight'].'px;';
				     ...
				     if ((isset($options[$name])) && esc_url( $options[$name])) { 
					  ...
					  echo '/>';
					   ...
					   _e('No image selected', 'BGE_kleider');
				     }				   
				    ?>		
				    </div>
				    <input type="hidden" name="BGE_kleider_theme_options[<?php echo $name; ?>_id]" id="image_<?php echo $name; ?>_id" 
					     value="<?php if ( isset( $options[$name."_id"] ) ) echo sanitize_key( $options[$name."_id"] ) ; ?>" />
				    
				    <input type="text" name="BGE_kleider_theme_options[<?php echo $name; ?>]" id="image_<?php echo $name; ?>" value="<?php if ( isset( $options[$name] ) ) echo esc_attr( $options[$name] ) ; ?>" />
				    <input class="button" name="image_button_<?php echo $name; ?>" id="image_button_<?php echo $name; ?>" value="<?php _e('Add Image', 'BGE_kleider'); ?>" />
				    <small><a href="#" class="image_remove_<?php echo $name; ?>"><?php _e( "Remove image", 'BGE_kleider' );?></a></small>
				    <?php if (isset($value['default']) && (filter_var($value['default'], FILTER_VALIDATE_URL))) { ?>
					<small><a href="#" class="image_reset_<?php echo $name; ?>"><?php _e( "Reset to default", 'BGE_kleider' );?></a></small>
				    <?php } ?>
					...
					jQuery('.image_remove_<?php echo $name; ?>').click(function()   {
						...
						jQuery('.showimg_<?php echo $name; ?>').html('<?php _e('No image selected', 'BGE_kleider'); ?>');
						return false;
					});
				    });
				    <?php if (isset($value['default']) && (filter_var($value['default'], FILTER_VALIDATE_URL))) { ?>
				    ...
				    <?php } ?>
                                    ...
                                    echo "\t\t\t";
                                    echo "<input class=\"number\" size=\"5\" id=\"BGE_kleider_theme_options[$name]\" 
                                            type=\"number\" name=\"BGE_kleider_theme_options[$name]\" 
				   ...
				   echo "\t\t\t";                                    
					...
					echo "\">\n";
					    ...
					    name="BGE_kleider_theme_options['.$name.']"';
                                        if ( $src == $options[$name] ) {
				   ...
				   echo "\t\t\t";                      
                                    foreach ( $liste as $option ) {    
                                        ...
                                        <label class="plakattile">
                                            ...
                                            <input type="checkbox" name="BGE_kleider_theme_options[<?php echo $name?>][]" 
				   ...
				   echo "\t\t\t";                      
					...
					$value = '';
					 ...
					 if (($active==1)  
					    ...
					    <label for="BGE_kleider_theme_options[<?php echo $name?>][<?php echo $entry?>][active]" class="<?php echo $entry?>" style="width: 120px; display: inline-block;">
                                            <input type="checkbox" 
						   id="BGE_kleider_theme_options[<?php echo $name?>][<?php echo $entry?>][active]"
						   name="BGE_kleider_theme_options[<?php echo $name?>][<?php echo $entry?>][active]" 
                                                   value="1" <?php echo $checked; ?>>                                                                                               
					    <?php echo $liste[$entry]['name'] ?>
					    </label>
					    
                                            <input id="BGE_kleider_theme_options[<?php echo $name?>][<?php echo $entry?>][content]" 
                                            type="text" name="BGE_kleider_theme_options[<?php echo $name?>][<?php echo $entry?>][content]" 
					...
					echo "<p>".$label."</p>\n";
                                    ...
                                    echo "\t\t\t";
                                    echo "<select id=\"$name\" name=\"BGE_kleider_theme_options[$name]\">\n";
					...
					$value = '';                                      
                                         ...
                                         ?>       
					    <label for="BGE_kleider_theme_options[<?php echo $name?>][<?php echo $entry?>]" >                                                
                                            <input type="checkbox" 
						   id="BGE_kleider_theme_options[<?php echo $name?>][<?php echo $entry?>]"
						   name="BGE_kleider_theme_options[<?php echo $name?>][<?php echo $entry?>]" 
					...
					echo "<p>".$label."</p>\n";
                                    ...
                                    echo "\t\t\t";
                                    echo "<select name=\"BGE_kleider_theme_options[$name]\">\n";
                                    foreach($liste as $i => $value) {   
                                        echo "\t\t\t\t";
                        ...
                        _e( 'Option not defined', 'BGE_kleider' );
                    }
                ?>
        ...
        <p class="submit">
                <input type="submit" class="button-primary" value="<?php _e( 'Update', 'BGE_kleider' ); ?>" />
        </p>
</form>               
</div>

</div> <!-- end: .BGE_kleider-optionen -->      
<?php
}
...
function theme_options_validate( $input ) {
    ...
    $options = get_option( 'BGE_kleider_theme_options' );
    
    $saved = (array) get_option( 'BGE_kleider_theme_options' );	
        //    $options= $saved;
	...
	delete_option('BGE_kleider_theme_options');
