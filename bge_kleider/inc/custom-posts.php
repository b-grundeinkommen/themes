...
function BGE_kleider_custom_post_linktipps() {    
	$labels = array(
		'name'               => __( 'Bookmarks', 'BGE_kleider' ),
		'singular_name'      => __( 'Bookmark', 'BGE_kleider' ),
		
	);
	$args = array(
	    'labels'		=> $labels,
	    'description'	=> __( 'Add or manage bookmark entries', 'BGE_kleider' ),
	    'public'		=> false,
	    'show_ui'		=> true,
	    'menu_position'	=> 8,
...
add_action( 'init', 'BGE_kleider_custom_post_linktipps' );


function BGE_kleider_taxonomies_linktipps() {
	$labels = array();
	$args = array(
		'labels'	=> $labels,
		'hierarchical'	=> true,
...
add_action( 'init', 'BGE_kleider_taxonomies_linktipps' );

function BGE_kleider_linktipp_metabox() {
    add_meta_box( 
        'linktipp_metabox',
        __( 'Description and target URL', 'BGE_kleider' ),
...
function linktipp_metabox_content( $post ) {
	...
	<p>
		<label for="linktipp_url"><?php _e( "Enter target URL", 'BGE_kleider' ); ?>:</label>
		<br />
	...
	<p>
		<label for="linktipp_text"><?php _e( "Enter an optional and short target description", 'BGE_kleider' ); ?>:</label>
		<br />
	...
	<p>
	    <label for="linktipp_image"><?php _e( "Image for bookmark (either by media library or external URL)", 'BGE_kleider' ); ?>:</label>
		...
		?>
	     ...
	     <input type="button" id="linktipp_image-button" class="button" value="<?php _e( "Select image", 'BGE_kleider' ); ?>" />
	    <small> <a href="#" class="custom_clear_image_button"><?php _e( "Remove image", 'BGE_kleider' ); ?></a></small> 
	</p>
	
	<p>
		<label for="linktipp_untertitel"><?php _e( "Optional pretitle", 'BGE_kleider' ); ?>:</label>
		<br />
...
add_action( 'add_meta_boxes', 'BGE_kleider_linktipp_metabox' );
	   ...
	   $out .= "\n";
		   ...
		   $out .= '</a>';
			...
			if (isset($linktipp_text)) {
				$out .=  BGE_kleider_make_nice_links($linktipp_text);
			}
	   $out .= "</article>\n"; 
...
function linktipps_shortcode( $atts ) {
	...
	$links = new WP_Query( $args );
		if( $links->have_posts() ) {
			...
			$out .= __('No bookmarks found.', 'BGE_kleider');
			$out .= "</p></section>\n";
		}
