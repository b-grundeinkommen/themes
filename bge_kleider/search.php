  ...
  <div class="row">
	...
	<?php
	    ...
	    if (($options['aktiv-platzhalterbilder-indexseiten']==1) && (isset($options['src-default-symbolbild-search']))) {  
			...
			$attribs = BGE_kleider_get_image_attributs($options['src-default-symbolbild-search_id']);
		    } else {
		...
		?>    		    		    		        
		   <h1 class="post-title"><span><?php printf( __( 'Search entries for "%s"', 'BGE_kleider' ), '' .get_search_query() . '' ); ?></span></h1>
	...
	if ($search->have_posts() ) { 
          ...
          while ($search->have_posts()) : $search->the_post();
              $out = BGE_kleider_search_teaser($options['suche-excerptlength'],0,1,$s);        
              if (isset($out)) {
                $output .= $out;
              }
          endwhile;
          ...
          if (isset($output)) {   
              ...
              echo "<p>";
              if ($total_results>1) {
                printf( __( 'Found %s matching hits.', 'BGE_kleider' ), $total_results );
              } else {
                  _e('Found one matching hit.','BGE_kleider');
              }
              echo "</p>\n";         
         ...
         if (  $wp_query->max_num_pages > 1 ) { ?>
                <div class="archiv-nav">
                    ...
                    if ($page>1) {
                        echo '<li class="prev">';
                        echo '<a href="'.get_pagenum_link(1).'">'.__( '&larr; First page', 'BGE_kleider' ).'</a>';
                        echo "</li>\n";
                    }
                    echo '<li class="pages">';
                    BGE_kleider_paging_bar($total_results,$options['suche-treffer_pro_seite']);
                    echo "</li>\n";   
                    $last = intval($total_results/$options['suche-treffer_pro_seite']);
                    if ($page < $last) {
                        
                        echo '<li class="next">';
                        echo '<a href="'.get_pagenum_link($last).'">'.__( 'Last page &rarr;', 'BGE_kleider' ).'</a>';
                        echo "</li>\n";
                    }
                    ?>                                
                </ul></div> 
         <?php 
                        ...
                        <p>
                            <?php _e("No matching pages or entries found. Please try with another term.", 'BGE_kleider'); ?>
                            
                        </p>
                        <?php get_search_form(); ?>
                        
                        <p>
                            <?php _e("Alternativly try the following links:", 'BGE_kleider'); ?>
                                      
                        </p>
                        
                        <div class="widget">
                            <h3><?php _e("Archive by month", 'BGE_kleider'); ?></h3>                           
                        ...
                        if ($posttags) { ?>                          
                         <div  class="widget">
                            <h3><?php _e("Entries by tags", 'BGE_kleider'); ?></h3>    
                            <div class="tagcloud">
                        ...
                        <?php } ?>
                        <div class="widget">
                        <h3><?php _e("Categories", 'BGE_kleider'); ?></h3>
                        <ul>                            
