<?php
/**
 * BGE_kleider 3 Theme Optionen
 *
 * @source http://github.com/xwolfde/BGE_kleider
 * @creator xwolf
 * @version 3.3
 * @licence GPL 2.0 
 */
...
load_theme_textdomain('BGE_kleider', get_template_directory() . '/languages');
$locale = get_locale();
...
$options = BGE_kleider_initoptions();
...
if ( ! function_exists( 'BGE_kleider_setup' ) ):
function BGE_kleider_setup() {
	...
	if ( ! isset( $content_width ) )   $content_width = $defaultoptions['content-width'];
       ...
       $args = array(
	    ...
	    'wp-head-callback'       => 'BGE_kleider_custom_background_cb',	
	);
       
	/**
	 * BGE_kleider custom background callback.
	 *
	 */
	function BGE_kleider_custom_background_cb() {
                 global $defaultoptions;
                 global $options;
	...
	if ( function_exists( 'add_image_size' ) ) { 
        ...
        register_nav_menus( array(
                'primary' => __( 'Main navigation', 'BGE_kleider' ),
                'top' => __( 'Website Menu', 'BGE_kleider' ),
                'sub' => __( 'Tecnical navigation', 'BGE_kleider' ),
        ) );
...
add_action( 'after_setup_theme', 'BGE_kleider_setup' );

require( get_template_directory() . '/inc/widgets.php' );

function BGE_kleider_scripts() {
    global $options;
    global $defaultoptions;
	...
	if (($userstyle==0) || (($userstyle==1) && ($options['stylefile-position']<3))) {
		...
		$theme  = wp_get_theme();
		wp_register_style( 'BGE_kleider', get_bloginfo( 'stylesheet_url' ), false, $theme['Version'] );
		wp_enqueue_style( 'BGE_kleider' );
	    }     		
	}
	if (($userstyle==1) && ($options['stylefile-position'] > 1)) {
	    wp_enqueue_style( 'stylefile', wp_get_attachment_url($options['aktiv-stylefile']));	  
	...
	if (($userstyle==0) || (($userstyle==1) && ($options['stylefile-position']!=4))) {
	    ...
	    if ( is_singular() ) {
		$nosidebar = get_post_meta( get_the_ID(), 'BGE_kleider_nosidebar', true );
...
add_action('wp_enqueue_scripts', 'BGE_kleider_scripts');
function BGE_kleider_dequeue_fonts() {
         wp_dequeue_style( 'twentytwelve-fonts' );
      }
add_action( 'wp_enqueue_scripts', 'BGE_kleider_dequeue_fonts', 11 );

function BGE_kleider_addfonts() {
  global $options;
  ...
  if ((isset($options['fonts-content'])) && ($options['fonts-content'] != 'none')) {
      ...
      if (isset($default_fonts[$setfont]['webfont'])
        ...
        $output .= '@font-face { font-family: FontBGE_kleiderDefault; local: '.$setfont.'; src: ';
          ...
          $output .= "body,.defaultfont,.teaserlinks ul li a span { font-family: FontBGE_kleiderDefault; }\n";
  ...
  if ((isset($options['fonts-headers'])) && ($options['fonts-headers'] != 'none')) {
      ...
      if (isset($default_fonts[$setfont]['webfont'])
              && ($default_fonts[$setfont]['webfont']==1)) {
        $output .= '@font-face { font-family: FontBGE_kleiderHeadlines; local: '.$setfont.'; src: ';
          if (isset($default_fonts[$setfont]['eot'])) {
              $output .= 'url('.get_template_directory_uri().$default_fonts[$setfont]['eot'].') format("embedded-opentype")';
              $seturl = 1;
          }
          ...
          $output .= "h1,h2,h3,h4,h5,h6,.headlinefont,.tagcloud,.post-nav a,.first-startpage-widget-area li a { font-family: FontBGE_kleiderHeadlines; }\n";
  ...
  if ((isset($options['fonts-menuheaders'])) && ($options['fonts-menuheaders'] != 'none')) {
      ...
      if (isset($default_fonts[$setfont]['webfont'])
              && ($default_fonts[$setfont]['webfont']==1)) {
        $output .= '@font-face { font-family: FontBGE_kleiderMenuHeadlines; local: '.$setfont.'; src: ';
          if (isset($default_fonts[$setfont]['eot'])) {
              $output .= 'url('.get_template_directory_uri().$default_fonts[$setfont]['eot'].') format("embedded-opentype")';
              $seturl = 1;
          }
          ...
          $output .= ";}\n";    
          $output .= ".menufont, .nav-main ul.menu li a,.cifont,.sticker ul li,.teaserlinks ul li a { font-family: FontBGE_kleiderMenuHeadlines; }\n";  
      } else {
...
add_action('wp_head', 'BGE_kleider_addfonts');


function BGE_kleider_addaltbodybackground() {
  global $options;
...
add_action('wp_head', 'BGE_kleider_addaltbodybackground');

function BGE_kleider_addmetatags() {
    global $options;
    ...
    if ((isset($options['favicon-file'])) && ($options['favicon-file_id']>0 )) {	 
        ...
        $output .=  '<link rel="apple-touch-icon" href="'.get_BGE_kleider_template_uri().'/apple-touch-icon.png">'."\n";
        $output .=  '<link rel="shortcut icon" href="'.get_BGE_kleider_template_uri().'/favicon.ico">'."\n";
    }
    echo $output;
}
add_action('wp_head', 'BGE_kleider_addmetatags',1);
...
function BGE_kleider_load_open_graph() {
    global $post;
    global $options;
    ...
    if ( is_front_page() ) { // Alternativ is_home
        ...
        echo '<meta property="og:url" content="' .get_BGE_kleider_home_url() . '" />'."\n";
        ...
        if (!empty($options['meta-description'])) {
            ...
            echo '<meta name="twitter:description" content="' . esc_attr( strip_tags(get_BGE_kleider_custom_excerpt(180, 0, 1, 2),'') ) . '" />'."\n";
        }
    }
    ...
    elseif ( is_singular() ) {
        ...
        echo '<meta property="og:description" content="' . esc_attr( strip_tags(get_BGE_kleider_custom_excerpt($options['open_graph_excerptlength'], 0, 1, 2),'') ) . '" />';
...
add_action( 'wp_head', 'BGE_kleider_load_open_graph' );


function get_BGE_kleider_home_url() {
     if (is_ssl()) {
...
function BGE_kleider_avatar ($avatar_defaults) {
    global $defaultoptions;
    $myavatar =  $defaultoptions['src-default-avatar']; 
    $avatar_defaults[$myavatar] = "BGE_kleider";
    return $avatar_defaults;
}
add_filter( 'avatar_defaults', 'BGE_kleider_avatar' );
...
if ( ! function_exists( 'get_BGE_kleider_options' ) ) :
/*
 * Erstes Bild aus einem Artikel auslesen, wenn dies vorhanden ist
 */
function get_BGE_kleider_options( $field ){
    global $defaultoptions;	    
    if (!isset($field)) {
	$field = 'BGE_kleider_theme_options';
    }
...
function BGE_kleider_initoptions() {
    global $defaultoptions;
    global $default_toplink_liste;
    // $doupdate = 0;
    
    $oldoptions = get_option('BGE_kleider_theme_options');
    ...
    // if ($doupdate==1) {
	// update_option('BGE_kleider_theme_options', $newoptions);
    // }
    return $newoptions;
}

if ( ! function_exists( 'BGE_kleider_get_image_attributs' ) ) :
    function BGE_kleider_get_image_attributs($id=0) {
        $precopyright = __('Image:','BGE_kleider').' ';
        if ($id==0) return;
...
if ( ! function_exists( 'BGE_kleider_filter_wp_title' ) ) :   
/*
 * Sets the title
 */    
function BGE_kleider_filter_wp_title( $title, $separator ) {
        ...
        if ( is_search() ) {
                $title = sprintf( __( 'Search results for %s', 'BGE_kleider' ), '"' . get_search_query() . '"' );
                if ( $paged >= 2 )
                        $title .= " $separator " . sprintf( __( 'Page %s', 'BGE_kleider' ), $paged );
        ...
        if ( $paged >= 2 || $page >= 2 )
                $title .= " $separator " . sprintf( __( 'Page %s', 'BGE_kleider' ), max( $paged, $page ) );

        return $title;
}
endif;
add_filter( 'wp_title', 'BGE_kleider_filter_wp_title', 10, 2 );


function BGE_kleider_excerpt_length( $length ) {
	global $defaultoptions;
        return $defaultoptions['teaser_maxlength'];
}
add_filter( 'excerpt_length', 'BGE_kleider_excerpt_length' );

function BGE_kleider_continue_reading_link() {
        return ' <a class="nobr" title="'.strip_tags(get_the_title()).'" href="'. get_permalink() . '">' . __( 'Continue reading <span class="meta-nav">&rarr;</span>', 'BGE_kleider' ) . '</a>';
}

function BGE_kleider_auto_excerpt_more( $more ) {
        return ' &hellip;' . BGE_kleider_continue_reading_link();
}
add_filter( 'excerpt_more', 'BGE_kleider_auto_excerpt_more' );


function BGE_kleider_custom_excerpt_more( $output ) {
       if ( has_excerpt() && ! is_attachment() ) {      
                $output .= BGE_kleider_continue_reading_link();
        }
        return $output;
}
add_filter( 'get_the_excerpt', 'BGE_kleider_custom_excerpt_more' );



function BGE_kleider_remove_gallery_css( $css ) {
        return preg_replace( "#<style type='text/css'>(.*?)</style>#s", '', $css );
}
add_filter( 'gallery_style', 'BGE_kleider_remove_gallery_css' );
...
if ( ! function_exists( 'BGE_kleider_comment' ) ) :
/**
 * Template for comments and pingbacks.
 */
function BGE_kleider_comment( $comment, $args, $depth ) {
            ...
            <article <?php echo $addclass;?> itemprop="comment" itemscope itemtype="http://schema.org/UserComments">
              <header>  
                ...
                <div class="comment-author vcard" itemprop="creator" itemscope itemtype="http://schema.org/Person">
                    ...
                    printf( __( '%s <span class="says">commented at</span>', 'BGE_kleider' ), sprintf( '<cite class="fn" itemprop="name">%s</cite>', get_comment_author_link() ) ); 
                    ?>
                </div><!-- .comment-author .vcard -->
                <?php if ( $comment->comment_approved == '0' ) : ?>
                        <em><?php _e( 'Comment waits for approval.', 'BGE_kleider' ); ?></em>
                        <br />
                <?php endif; ?>
                ...
                <div class="comment-meta commentmetadata"><a itemprop="url" href="<?php echo esc_url( get_comment_link( $comment->comment_ID ) ); ?>"><time itemprop="commentTime" datetime="<?php comment_time('c'); ?>">
                    <?php
                       ...
                       printf( __( '%1$s at %2$s', 'BGE_kleider' ), get_comment_date(),  get_comment_time() ); ?></time></a> Folgendes:<?php edit_comment_link( __( '(Edit)', 'BGE_kleider' ), ' ' );
                    ?>
        ...
        <li class="post pingback">
                <p><?php _e( 'Pingback:', 'BGE_kleider' ); ?> <?php comment_author_link(); edit_comment_link( __('Edit', 'BGE_kleider'), ' ' ); ?></p>
        <?php
                        break;
        endswitch;
}
...
function BGE_kleider_remove_recent_comments_style() {
        global $wp_widget_factory;
        remove_action( 'wp_head', array( $wp_widget_factory->widgets['WP_Widget_Recent_Comments'], 'recent_comments_style' ) );
}
add_action( 'widgets_init', 'BGE_kleider_remove_recent_comments_style' );

if ( ! function_exists( 'BGE_kleider_post_teaser' ) ) :
/**
 * Erstellung eines Artikelteasers
 */
function BGE_kleider_post_teaser($titleup = 1, $showdatebox = 1, $showdateline = 0, $teaserlength = 200, $thumbfallback = 1, $usefloating = 0, $titlenum = 2) {
  global $options;
  global $post;
  ...
  if (($showdatebox>0)  && ($showdatebox<5)) {
       ...
       // Generate Thumb/Pic or Video first to find out which class we need
		...
		$thumbnailcode = get_the_post_thumbnail($post->ID, 'teaser-thumb');
            ...
            $firstpic = get_BGE_kleider_firstpicture();
            $firstvideo = get_BGE_kleider_firstvideo();
  ...
  $htmltitleend = '</h'.$titlenum.'>';
   $subtitle =  get_post_meta( $post->ID, 'BGE_kleider_subtitle', true );
	...
	if ($showdatebox==0) {		 
	      ...
	      if (($num_comments>0) || ( $options['zeige_commentbubble_null'])) { 
                    ...
                    if ($num_comments>0) {
                        $out .= __('Comments', 'BGE_kleider' ).'</span></a>';
                    } else {
                        $out .= __('Comment', 'BGE_kleider' ).'</span></a>';
                    }
                    $out .= "</div>\n"; 
	       }	
	...
	if ($titleup==0) {  
	 ...
	 if (($showdatebox!=0) && ($showdateline==1)) {  
	    ...
	    $out .=  BGE_kleider_post_pubdateinfo(0); 
	    $out .= "</p>\n";	  	  
	 }
	   
	 $out .= get_BGE_kleider_custom_excerpt($teaserlength,1,1,$options['continuelink']); 
	 if ($showdatebox<5) {	 
            $out .= "</div>\n";    	
...
if ( ! function_exists( 'BGE_kleider_category_teaser' ) ) :
/**
 * Erstellung eines Artikelteasers
 */
function BGE_kleider_category_teaser() {
  global $options;
  global $post;
    ...
    $out .=  BGE_kleider_post_pubdateinfo(0); 
    $out .= "</p>\n";	  	  	 
    $out .= get_BGE_kleider_custom_excerpt($options['categoryindex-teaserlength'],1,1,$options['continuelink']); 
    $out .= '<div class="p3-ie-clearing">&nbsp;</div>';	
    $out .= "</article>\n";
    $out .= '</div>';     
    $out .= "</section>\n"; 
...
if ( ! function_exists( 'BGE_kleider_search_teaser' ) ) :
/**
 * Suchausgabe
 */
function BGE_kleider_search_teaser($teaserlength = 250, $withthumb = 1, $aslist = 1, $search = '') {
  global $options;
  global $post;
    ...
    $out .= '<div>'; 
    $out .= $leftbox;  
    $excerpt = get_BGE_kleider_custom_excerpt($teaserlength, 1, 1, 2);  
    if (trim($search) !== '') {
    ...
    $out .= $excerpt;     
    $out .= '<p class="meta">';    
    $out .= '<span class="date">'.__('Created at:','BGE_kleider').' '.BGE_kleider_post_pubdateinfo(0).'.</span> '; 
    $typ =get_post_type();
    if ($typ == 'post') {
         $out .= '<span class="type">'.__('Type: Entry','BGE_kleider').'.</span> '; 
    } elseif ($typ=='page') {
         $out .= '<span class="type">'.__('Type: Page','BGE_kleider').'.</span> '; 
    } 
    $out .= "</p>\n";    
    $out .= '</div>';     
    if ($aslist==1) {
...
if ( ! function_exists( 'BGE_kleider_post_datumsbox' ) ) :
/**
 * Erstellung der Datumsbox
 */
function BGE_kleider_post_datumsbox() {
    global $options;
...
if ( ! function_exists( 'BGE_kleider_post_pubdateinfo' ) ) :
/**
 * Fusszeile unter Artikeln: Ver&ouml;ffentlichungsdatum
 */
function BGE_kleider_post_pubdateinfo($withtext = 1) {
    $out = '';
    if ($withtext==1) {
	$out .= '<span class="meta-prep">';
        $out .= __('Publiced at', 'BGE_kleider' );
	$out .= '</span> ';
    }
...
if ( ! function_exists( 'BGE_kleider_post_autorinfo' ) ) :
/**
 * Fusszeile unter Artikeln: Autorinfo
 */
function BGE_kleider_post_autorinfo() {
    $out = ' <span class="meta-prep-author">'.__('Author','BGE_kleider').':</span> ';
...
if ( ! function_exists( 'BGE_kleider_post_taxonominfo' ) ) :
/**
 * Fusszeile unter Artikeln: Taxonomie
 */
function BGE_kleider_post_taxonominfo() {
         $tag_list = get_the_tag_list( '', ', ' );
        if ( $tag_list ) {
                $posted_in = __( 'Category: <span itemprop="articleSection">%1$s</span>. Tags: <span itemprop="keywords">%2$s</span>. <br><a href="%3$s" title="%4$s" rel="bookmark" itemprop="url">Permalink</a> for this entry.', 'BGE_kleider' );
        } elseif ( is_object_in_taxonomy( get_post_type(), 'category' ) ) {
                $posted_in = __( 'Category: <span itemprop="articleSection">%1$s</span>. <br><a href="%3$s" title="%4$s" rel="bookmark" itemprop="url">Permalink</a> for this entry.', 'BGE_kleider' );
        } else {
                $posted_in = __( '<a href="%3$s" title="%4$s" rel="bookmark" itemprop="url">Permalink</a> for this entry.', 'BGE_kleider' );
        }
...
function BGE_kleider_change_mce_options($initArray) {
...
add_filter('tiny_mce_before_init', 'BGE_kleider_change_mce_options');



class BGE_kleider_Menu_Walker extends Walker_Nav_Menu {
...
if ( ! function_exists( 'get_BGE_kleider_socialmediaicons' ) ) :
/**
 * Displays Social Media Icons
 */
function get_BGE_kleider_socialmediaicons( $darstellung = 1 ){
    global $options;
    ...
    if ($zeigeoption ==2) {    
           /* Links an der Seite */
            echo '<nav id="socialmedia_iconbar" aria-label="'.__('Social Media','BGE_kleider').'">';
    } else {
	    echo '<nav id="socialmedia_top" aria-label="'.__('Social Media','BGE_kleider').'">';
    }
...
if ( ! function_exists( 'get_BGE_kleider_seitenmenu' ) ) :
/*
 * Anzeige des Sidebar-Menus
 */
function get_BGE_kleider_seitenmenu( $zeige_sidebarpagemenu = 1 , $zeige_subpagesonly =1 , $seitenmenu_mode = 0 ){
  global $post;
  $sidelinks = '';
    if ($zeige_sidebarpagemenu==1) {   
            if (($seitenmenu_mode == 1) || (!has_nav_menu( 'primary' ))) {
                    ...
                    if ($zeige_subpagesonly==1) {
                            wp_nav_menu( array('depth' => 0, 'container_class' => 'menu-header subpagesonly', 'theme_location' => 'primary', 'walker'  => new BGE_kleider_Menu_Walker()) );      
                    } else { 
                            wp_nav_menu( array('depth' => 0, 'container_class' => 'menu-header', 'theme_location' => 'primary', 'walker'  => new BGE_kleider_Menu_Walker()) );      
                    }
		}
    }
...
if ( ! function_exists( 'get_BGE_kleider_firstpicture' ) ) :
/*
 * Erstes Bild aus einem Artikel auslesen, wenn dies vorhanden ist
 */
function get_BGE_kleider_firstpicture($relative = true){
    global $post;
    $first_img = '';
    ob_start();
    ob_end_clean();
   ...
   if ((is_array($matches)) && (isset($matches[1]))) {
        ...
        if (!empty($first_img)){
            if ($relative) {
                $first_img = BGE_kleider_make_link_relative($first_img);
            }
...
if ( ! function_exists( 'get_BGE_kleider_firstvideo' ) ) :
/*
 * Erstes Bild aus einem Artikel auslesen, wenn dies vorhanden ist
 */
function get_BGE_kleider_firstvideo($width = 300, $height = 169, $nocookie =1, $searchplain =1){
    global $post;
    ob_start();
    ob_end_clean();
    $matches = array();
...
if ( ! function_exists( 'get_BGE_kleider_custom_excerpt' ) ) :
/*
 * Erstellen des Extracts
 */
function get_BGE_kleider_custom_excerpt($length = 0, $continuenextline = 1, $removeyoutube = 1, $alwayscontinuelink = 0){
  global $options;
  global $post;
  global $defaultoptions;
  ...
  if (has_excerpt()) {
      ...
      $excerpt = get_the_content();
       if (!isset($excerpt)) {
          $excerpt = __( 'No content', 'BGE_kleider' );
        }
  }
  if ($length==0) {
  ...
  if (mb_strlen($excerpt)<5) {
      $excerpt = '<!-- '.__( 'No entry for this post', 'BGE_kleider' ).' -->';
  }
  ...
  if ($alwayscontinuelink < 2) {
	  ...
	  $the_str .= BGE_kleider_continue_reading_link();
      }
  }
  $the_str .= '</p>';
  return $the_str;
...
if ( ! function_exists( 'BGE_kleider_fetch_feed' ) ) :
/*
 * Feet holen mit direkter Angabe der SimplePie-Parameter
 */
function BGE_kleider_fetch_feed($url,$lifetime=0) {
    global $defaultoptions;
    global $options;
...
*/
function BGE_kleider_make_nice_links($content) {
    ...
    foreach ($dom->getElementsByTagName('a') as $node) {
        ...
        if ($url_scheme == 'http' || $url_scheme == 'https') {
            $node->setAttribute('href', BGE_kleider_make_link_relative($url));
            if (!$node->hasAttribute('class') && !BGE_kleider_is_internal_link($url)) {
    ...
    foreach ($dom->getElementsByTagName('img') as $node) {
        ...
        $node->setAttribute('src', BGE_kleider_make_link_relative($url));
    }
...
add_filter('the_content', 'BGE_kleider_make_nice_links');

 
 function get_BGE_kleider_template_uri () {
...
function rw_relative_urls() {
   ...
   //     'post_link',
    ...
    foreach ($filters as $filter) {
        add_filter($filter, 'BGE_kleider_make_link_relative');
    }
}

function BGE_kleider_is_internal_link($url) {
...
function BGE_kleider_make_link_relative($url) {
    if (BGE_kleider_is_internal_link($url)) {
...
function BGE_kleider_breadcrumb() {
  global $defaultoptions;
  
  $delimiter	= $defaultoptions['breadcrumb_delimiter']; // = ' / ';
  $home		= $defaultoptions['breadcrumb_homelinktext']; // __( 'Startseite', 'BGE_kleider' ); // text for the 'Home' link
  ...
  if ( !is_home() && !is_front_page() || is_paged() ) { 
	...
	echo $before . $pretitletextstart. __( 'Entry of category', 'BGE_kleider' ).$pretitletextend. ' "' . single_cat_title('', false) . '"' . $after;
	...
	echo $before .$pretitletextstart. __( 'Search for', 'BGE_kleider' ).$pretitletextend.' "' . get_search_query() . '"' . $after; 
    } elseif ( is_tag() ) {
	echo $before .$pretitletextstart. __( 'Entries with tag', 'BGE_kleider' ).$pretitletextend. ' "' . single_tag_title('', false) . '"' . $after; 
    } elseif ( is_author() ) {
	global $author;
	$userdata = get_userdata($author);
	echo $before .$pretitletextstart. __( 'Entry by', 'BGE_kleider' ).$pretitletextend.' '.$userdata->display_name . $after;
    } elseif ( is_404() ) {
 ...
 /*
    if ( get_query_var('paged') ) {
      ...
      echo __('Page', 'BGE_kleider') . ' ' . get_query_var('paged');
      if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) echo ')';
    }
 */
...
function BGE_kleider_header_style() {} 

function BGE_kleider_admin_style() {
    global $defaultoptions;
...
add_action( 'admin_enqueue_scripts', 'BGE_kleider_admin_style' );
 ...
 * Paging Function 
 */
function BGE_kleider_paging_bar($total = 1, $perpage =1) {
  /* Init */
  $count = $total;
...
function BGE_kleider_html_tag_schema() {
...
function BGE_kleider_tag_schema_org_name() {
    global $options;
...
function BGE_kleider_tag_schema_org_desc() {
    global $options;
...
add_filter('page_template', 'BGE_kleider_page_template');
function BGE_kleider_page_template($t) {
    $compatlist = array(
...
function featured_image_in_rss($content) {
    ...
    if(is_feed() && $options['feed-addthumbnail']) {  
        if (!isHTML($content)) {
            ...
            $firstpic = get_BGE_kleider_firstpicture(false);
...
function BGE_kleider_admin_init() {
...
add_action('admin_init', 'BGE_kleider_admin_init'); 