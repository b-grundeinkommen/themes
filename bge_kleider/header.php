...
<!--[if (gt IE 9)|!(IE)]><!--> <html <?php echo BGE_kleider_html_tag_schema(); ?> <?php language_attributes(); ?>> <!--<![endif]-->
<head>
...
<body <?php body_class(); ?>>
	...
	<ul id="top" class="nav skiplinks">		
	    <li><a id="skiplink-nav" href="#nav"><?php _e( 'Jump to navigation.', 'BGE_kleider' ); ?></a></li>
	    <li><a id="skiplink-content" href="#main-content"><?php _e( 'Jump to content.', 'BGE_kleider' ); ?></a></li>
	    <?php if ( $options['aktiv-suche'] == "1" ){ ?><li><a id="skiplink-search" href="#searchform"><?php _e( 'Jump to search form.', 'BGE_kleider' ); ?></a></li><?php } ?>
	</ul>
    </nav>
    <header class="section header">
        <div class="row">
                ...
                <div class="nav-top" role="navigation">				                                                        
                    <h2 class="skip"><?php _e( 'Service-Navigation', 'BGE_kleider' ); ?></h2>
                    <?php  
                    get_BGE_kleider_socialmediaicons(1);  
			    ...
			    <h2 class="skip"><?php _e("Search", 'BGE_kleider'); ?></h2>
			    <form method="get" class="searchform" action="<?php echo home_url('','relative'); ?>/" role="search">
				    <label class="skip" for="s"><?php _e("Searching for", 'BGE_kleider'); ?>:</label>
				    <input type="text" value="<?php the_search_query(); ?>" name="s" id="s" placeholder="<?php _e("Enter search term", 'BGE_kleider'); ?>"  
        ...
        <?php if ( ! is_home() ) { ?>
              ...
              <meta itemprop="name" content="<?php echo esc_attr(BGE_kleider_tag_schema_org_name()); ?>" />              
              <?php if ( ! is_home() ) { ?>
                  </a>
              <?php } if (strlen(trim(get_bloginfo( 'description' )))>1) { ?>
                  <p class="description slogan" itemprop="description"><?php echo esc_attr(BGE_kleider_tag_schema_org_desc()); ?></p>
              <?php } ?>
    </div>
                ...
                <div class="sticker" aria-hidden="true">
                        ...
                        <div class="skin">   
                           ...
                           <h2 class="skip"><?php _e( 'Sticker', 'BGE_kleider' ); ?></h2>                             
                           <ul>
                ...
                <nav aria-label="<?php _e( 'Navigation', 'BGE_kleider' ); ?>" class="nav-main" id="nav" role="navigation" itemscope itemtype="http://schema.org/SiteNavigationElement">
                            <h2 class="skip"><?php _e( 'Navigation', 'BGE_kleider' ); ?></h2>
                            <?php 
                            if ( has_nav_menu( 'primary' ) ) {
                                wp_nav_menu( array(  'theme_location' => 'primary', 'walker'  => new BGE_kleider_Menu_Walker()) );      
                            } else { ?>
...
<div id="content-body" class="with-breadcrumb">
	<div class="section breadcrumbs">
		...
		<?php if (function_exists('BGE_kleider_breadcrumb')) BGE_kleider_breadcrumb(); 
	    ?>
	    </div></div>
	</div>
<?php } else { ?>
