	...
	<?php
	    ...
	    if (($options['aktiv-platzhalterbilder-indexseiten']==1) && (isset($options['src-default-symbolbild-tag']))) {  
		 if (isset($options['src-default-symbolbild-tag_id']) && ($options['src-default-symbolbild-tag_id']>0)) {
			...
			$attribs = BGE_kleider_get_image_attributs($options['src-default-symbolbild-tag_id']);
		    } else {
		...
		?>    		    		    		        
		   <h1 class="post-title"><span><?php printf( __( 'Tag %s', 'BGE_kleider' ), '' . single_cat_title( '', false ) . '' ); ?></span></h1>
	  ...
	  <?php if (!(isset($image_url) && (strlen($image_url)>4))) { ?>
	    <h1 class="post-title"><span itemprop="name"><?php printf( __( 'Tag %s', 'BGE_kleider' ), '' . single_cat_title( '', false ) . '' ); ?></span></h1>	
            <?php }
