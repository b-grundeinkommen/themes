  ...
  <div class="row">
	...
	<?php 
	   if ( have_posts() ) while ( have_posts() ) : the_post();         
		...
		if (!(isset($image_url) && (strlen($image_url)>4))) { 		
		    if (($options['aktiv-defaultseitenbild']==1) && (isset($options['seiten-defaultbildsrc']))) {  
			 if (isset($options['seiten-defaultbildsrc_id']) && ($options['seiten-defaultbildsrc_id']>0)) {
				...
				$image_url = $image_url_data[0];
				$attribs = BGE_kleider_get_image_attributs($options['seiten-defaultbildsrc_id']);
			  } else {
	    ...
	    wp_link_pages( array( 'before' => '' . __( 'Pages:', 'BGE_kleider' ), 'after' => '' ) );                   
    ...
    <div class="content-aside">
      <div class="skin">                
	    ...
	    $custom_fields = get_post_custom();
	      ...
	      if (isset($custom_fields['BGE_kleider_sidebar-content'])) {
		    $text = $custom_fields['BGE_kleider_sidebar-content'][0];
