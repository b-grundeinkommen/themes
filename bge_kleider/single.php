  ...
  <div class="row">
	...
	<?php if ( have_posts() ) while ( have_posts() ) : the_post();         
	    ...
	    $image_alt = '';
            ...
            $sliderimage =  get_post_meta( get_the_ID(), 'BGE_kleider_slider_image', true );
            if ($sliderimage) {
                    $image_url_data = wp_get_attachment_image_src( $sliderimage, 'highslider');  
                    $image_url = $image_url_data[0];
                    $attribs = BGE_kleider_get_image_attributs($sliderimage);

            } elseif (has_post_thumbnail()) { 
		$thumbid = get_post_thumbnail_id(get_the_ID());
		$image_url_data = wp_get_attachment_image_src( $thumbid, 'full');
		$image_url = $image_url_data[0];
		$attribs = BGE_kleider_get_image_attributs($thumbid);	
	    } 
		...
		if (($options['aktiv-artikelbild']==1) && (isset($options['artikelbild-src']))) {  
			...
			$image_url = $image_url_data[0];
			$attribs = BGE_kleider_get_image_attributs($options['artikelbild-src_id']);
		    } else {
	...
	<?php } ?>
	    ...
	    <?php 
	    $show_disclaimer = get_post_meta( get_the_ID(), 'BGE_kleider-show-post-disclaimer', true );
		...
		echo BGE_kleider_post_datumsbox();  
                ?>  
	      <article class="post-entry" itemprop="articleBody">
    
		<?php 
                 $subtitle =  get_post_meta( get_the_ID(), 'BGE_kleider_subtitle', true );
                if ($subtitle) {
		...
		<div class="post-meta"><p>
			...
			if ($options['aktiv-autoren']) BGE_kleider_post_autorinfo();             
			echo ' ';  
			BGE_kleider_post_taxonominfo();  
			?>                  
		      </p>
		</div>
		<div><?php edit_post_link( __( 'Edit', 'BGE_kleider' ), '', '' ); ?></div>
        </section>
	<div class="post-nav">
		<ul>
		<?php 
	    ...
	    <div class="post-nav">
	      <h3><?php _e("More entries:", 'BGE_kleider'); ?></h3>
	      <ul class="related">
		<?php do_action(
		...
		'after' => '</li>',
		'type' => 'post',
		'message' => __('No more entries in this category found.','BGE_kleider')
		)
		) ?>
          </ul>
	  </div>
	...
	?>
		...
		echo get_BGE_kleider_steckbrief();
