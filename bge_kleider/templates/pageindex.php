...
<div class="section content" id="main-content">
  <div class="row">
		...
		$category = get_term_by('ID',$cat, 'category');
                ...
                if( $my_query->have_posts() ) {
                  while ($my_query->have_posts()) : $my_query->the_post();
                    ...
                    $catfound = 1;
                    if ($i==0) {
                       	$topentry = BGE_kleider_category_teaser($my_query);
                        $i=1;
                    } else {
                  ...
                  if ($catfound==1) {
                      ...
                      if (strlen($liste)>0) {
                        ...
                        . '" title="' . sprintf( __( "Entries by category %s" , 'BGE_kleider'), $category->name ) 
                        . '" ' . '>'.__('More entries...','BGE_kleider').'</a>'.'</p>';
	...
	if ($foundarticles==0) { ?>
            ...
            <p>   
            <?php _e("Notice for authors and blog-admins: Top make pages show in this index, article-authors have to set categories on them.", 'BGE_kleider'); ?>
            </p>
            <?php get_search_form(); 
            echo "<hr>\n"; 
