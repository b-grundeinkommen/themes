    ...
    echo '</div></div>';    
       ...
       if (($options['aktiv-platzhalterbilder-indexseiten']==1) && (isset($options['src-default-symbolbild-category']))) {  
		 if (isset($options['src-default-symbolbild-category_id']) && ($options['src-default-symbolbild-category_id']>0)) {
			...
			$attribs = BGE_kleider_get_image_attributs($options['src-default-symbolbild-category_id']);
		    } else {
  ...
  <div class="section content" id="main-content">
	...
	<?php                 
        ...
        if (isset($image_url) && (strlen($image_url)>4)) { 
            ...
            ?>    		    		    		        
               <h1 class="post-title"><span><?php printf( __( 'Kategorie %s', 'BGE_kleider' ), '' . single_cat_title( '', false ) . '' ); ?></span></h1>
		   ...
		   <?php if (($options['category-teaser']==0) && isset($attribs["credits"]) && (strlen($attribs["credits"])>1)) {
               ...
               <?php                  
              if ($options['category-teaser'])  { 	  
                    echo '<h1 class="skip">'.__("Current entry", 'BGE_kleider').' ';
                    printf( __( 'Category %s', 'BGE_kleider' ), '' . single_cat_title( '', false ) . '' );
                    echo '</h1>';	    	     
              }
               echo '</div>';  
        } 
      ...
      if (( isset($options['category-num-article-fullwidth']))
		...
		$output =  BGE_kleider_post_teaser($options['category-teaser-titleup'],$options['category-teaser-datebox'],$options['category-teaser-dateline'],$options['category-teaser-maxlength'],$options['teaser-thumbnail_fallback'],$options['category-teaser-floating']);
      } else {
		$output =  BGE_kleider_post_teaser($options['category-teaser-titleup-halfwidth'],$options['category-teaser-datebox-halfwidth'],$options['category-teaser-dateline-halfwidth'],$options['category-teaser-maxlength-halfwidth'],$options['teaser-thumbnail_fallback'],$options['category-teaser-floating-halfwidth']);  
      }    
    ...
    <meta itemprop="name" content="<?php printf( __( 'Category %s', 'BGE_kleider' ), '' . single_cat_title( '', false ) . '' );?>" />
         <?php    
    ...
    <?php if (  $wp_query->max_num_pages > 1 ) : ?>
     <div class="archiv-nav"><p>
                <?php next_posts_link( __( '&larr; Older entries', 'BGE_kleider' ) ); ?>
                <?php previous_posts_link( __( 'Newer entries &rarr;', 'BGE_kleider' ) ); ?>
         </p></div>
    <?php endif; ?>             
                
                
    <?php if ( ! have_posts() ) : ?>
       <h2><?php _e("Nothing found", 'BGE_kleider'); ?></h2>
        <p>
            <?php _e("No matching pages or entries found. Please try to search with another term.", 'BGE_kleider'); ?>
        </p>
        <?php get_search_form(); ?>
        <hr>
     <?php endif;
